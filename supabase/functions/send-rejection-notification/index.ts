...
serve(async (req) => {
  ...
  try {
    const { spotId, spotName, userId, rejectionReason, type } = await req.json()

    if (!spotId || !spotName || !userId || !rejectionReason || type !== "rejected") {
      return new Response(
    ...
    const { error: emailError } = await supabaseClient.functions.invoke("send-email", {
      body: {
        to: userEmail,
        subject: "スポットが却下されました",
        html: `
          <h2>スポットが却下されました</h2>
          <p>あなたが投稿したスポット「${spotName}」が却下されました。</p>
          <p><strong>却下理由:</strong></p>
          <p>${rejectionReason}</p>
          <p>却下日時: ${new Date().toLocaleString("ja-JP")}</p>
          <p>スポットID: ${spotId}</p>
          <p>再度投稿する場合は、却下理由を参考に内容を修正してください。</p>
        `,
      },
    })
