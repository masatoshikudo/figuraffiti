...
- Phase1 MVP では、同じ場所に再設置しても `spot` は毎回新規作成する
- ただし将来拡張では、「同じ場所に何度出現したか」を追えるようにする余地を残す
- このときの「同じ場所」判定は、完全一致ではなく **半径10m以内** を同一地点バッファとして扱う想定
- `display_lat / display_lng` ではなく、実座標である `lat / lng` を基準に束ねる前提とする
- Phase2 以降では、よく出現する場所の分析、Legend演出、場所固有の物語化に活用できる可能性がある
