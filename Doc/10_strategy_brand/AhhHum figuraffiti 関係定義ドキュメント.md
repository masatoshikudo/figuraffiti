...
### 9.1 公開コピーで使わない内部用語一覧

公開コピーでは、内部設計を整理するためのラベルをそのまま出さない。  
迷った場合は、内部用語ではなく `AhhHum` と体験特徴に翻訳して書く。
...
- `参加メカニズム`
  - 設計説明では使用可
  - 公開では `参加のしかた` `遊び方` などに言い換える
- `運用主体`
  - 内部では使用可
  - 公開では `運営者` `アノニマスアーティスト` など文脈に応じて言い換える
- `ブランド接点`
  - 内部では使用可
  - 公開では `AhhHumを知る入口` `参加の入口` などに言い換える
- `参加導線`
  - 内部では使用可
  - 公開では `参加方法` `マップから始める` `記録ページへ進む` などに言い換える

短く言えば、公開コピーでは `内部構造の名前` を見せず、`ユーザーが体験する内容` を書く。
